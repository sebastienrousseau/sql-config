...
%global npm_bare      sql-config
%global sitelib       %{_prefix}/lib/node_modules

Name:           nodejs-sebastienrousseau-sql-config
Version:        0.0.7
Release:        1%{?dist}
Summary:        Shareable SQL configuration providing standardized SQLFluff and SQL formatter rules adhering to modern 2026 standards

License:        Apache-2.0 OR MIT
URL:            https://github.com/sebastienrousseau/sql-config
Source0:        %{name}-%{version}.tar.gz
BuildArch:      noarch
Requires:       nodejs >= 20

%description
@sebastienrousseau/sql-config is a shareable configuration preset with no runtime
...
install -pm 0644 .sqlfluff %{buildroot}%{sitelib}/%{npm_scope}/%{npm_bare}/
install -pm 0644 .sql-formatter.json %{buildroot}%{sitelib}/%{npm_scope}/%{npm_bare}/
