...
- Added a `native-validate` CI job that installs the real toolchain (sqlfluff, sql-formatter) and validates the shipped presets.
- Corrected the OpenSSF Best Practices badge, which pointed at another project.
## [0.0.1] - 2026-09-09

### Added

- Initial release of `@sebastienrousseau/sql-config` conforming to REPO-STANDARD.
- Standardized SQL configuration presets: `.sqlfluff`, `.sql-formatter.json`.
