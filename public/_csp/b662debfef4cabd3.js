...
lp=lm?'/sql-config/'+lm[1]:'/sql-config';
function load(){if(idx)return Promise.resolve();var sp=lm?'/sql-config/'+lm[1]+'/search-index.json':'/sql-config/search-index.json';return fetch(sp).then(function(r){return r.json()}).then(function(d){idx=d.entries||[]}).catch(function(){idx=[]})}
